...
export const SoldProductsPage: React.FC = () => {
  ...
  const handleEditOrder = (order: ApiOrder) => {
    ...
    if (order.is_anonymous) {
      ...
      const foundCustomer = customers.find(
        (c) => c.name === order.customer_fullname,
      );
    ...
    setPaymentMethod(
      order.payment_method as "cash" | "card" | "mixed" | "nasiya",
    );
  ...
  const handleDeleteOrder = async (orderId: number) => {
    ...
    try {
      ...
      toast.success(language === "uz" ? "Buyurtma o'chirildi" : "Заказ удален");
    } catch (error: any) {
  ...
  const getPaymentMethodBadge = (method: string) => {
    ...
    return (
      <Badge
        variant={variants[method] || "default"}
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
            ...
            <div className="text-right">
              ...
              <p className="text-xs text-gray-500">
                ...
                {formatCurrency(parseFloat(item.price) * item.quantity)} UZS
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-2 text-xs">
          ...
          <div>
            ...
            <span className="ml-1 font-medium">
              {formatCurrency(cutting.price)} UZS
            </span>
          </div>
          <div className="col-span-2">
            ...
            <span className="ml-1 font-medium">
              {formatCurrency(cutting.total_price)} UZS
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="mt-2 p-2 bg-green-50 dark:bg-green-900/20 rounded-lg">
        ...
        <div className="grid grid-cols-2 gap-2 text-xs">
          ...
          <div>
            ...
            <span className="ml-1 font-medium">
              {thickness.price ? formatCurrency(thickness.price) : "0"} UZS
            </span>
          </div>
          <div>
            <span className="text-gray-500">
              {language === "uz" ? "Eni" : "Ширина"}:
            </span>
            <span className="ml-1 font-medium">{banding.width} mm</span>
          </div>
          <div>
            <span className="text-gray-500">
              {language === "uz" ? "Bo'yi" : "Высота"}:
            </span>
          ...
          <div className="col-span-2">
            ...
            <span className="ml-1 font-medium">
              {formatCurrency(banding.total_price)} UZS
            </span>
          </div>
        </div>
      </div>
        ...
        <div className="flex items-center gap-2 bg-white dark:bg-gray-800 p-2 rounded-lg shadow-sm border dark:border-gray-700">
          ...
          <div className="flex items-center gap-1">
            ...
            <Input
              ...
              className="h-8 text-xs w-[140px] sm:w-[160px] bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 border-gray-300 dark:border-gray-600"
              max={new Date().toISOString().split("T")[0]}
            />
          </div>
          <Badge
            variant="outline"
            className="text-xs whitespace-nowrap dark:border-gray-600 dark:text-gray-300"
          >
            {filteredOrders.length} {language === "uz" ? "buyurtma" : "заказов"}
          </Badge>
        </div>
      </div>
      ...
      <Card>
        <CardHeader className="px-3 sm:px-4 py-2 sm:py-3">
          ...
          <CardDescription className="text-xs">
            {selectedDate
              ? language === "uz"
                ? `${format(new Date(selectedDate), "dd.MM.yyyy")} sanasidagi buyurtmalar`
                : `Заказы за ${format(new Date(selectedDate), "dd.MM.yyyy")}`
              : language === "uz"
          ...
          {isFetchingOrders ? (
            ...
            <div className="text-center py-8 sm:py-12">
              ...
              <p className="mt-2 sm:mt-4 text-xs sm:text-sm text-gray-500">
                {language === "uz"
                  ? `${format(new Date(selectedDate), "dd.MM.yyyy")} sanasida buyurtmalar yo'q`
                  : `Нет заказов за ${format(new Date(selectedDate), "dd.MM.yyyy")}`}
              </p>
              <Button
                variant="link"
                size="sm"
                        ...
                        <div className="space-y-2">
                          <div className="flex flex-wrap items-center gap-2">
                            ...
                            {isAnonymous ? (
                              ...
                              <Badge
                                variant="default"
                                className="flex items-center gap-1 text-xs max-w-[200px]"
                              >
                            ...
                            {order.cutting && (
                              <Badge
                                variant="outline"
                                className="bg-blue-50 text-blue-700 dark:bg-blue-900/20 dark:text-blue-300 border-blue-200 dark:border-blue-800 text-[10px] sm:text-xs"
                              >
                            ...
                            {order.banding && (
                              <Badge
                                variant="outline"
                                className="bg-green-50 text-green-700 dark:bg-green-900/20 dark:text-green-300 border-green-200 dark:border-green-800 text-[10px] sm:text-xs"
                              >
                                ...
                                <span className="ml-1 font-medium text-red-600">
                                  {formatCurrency(
                                    parseFloat(order.total_price) -
                                      parseFloat(order.covered_amount),
                                  )}{" "}
                                  UZS
                                </span>
                              </div>
                          ...
                          {(order.cutting || order.banding) && (
                            <div>
                              ...
                              <div className="space-y-2">
                                {/* Cutting Service */}
                                {order.cutting &&
                                  renderCuttingDetails(order.cutting)}

                                {/* Banding Service */}
                                {order.banding &&
                                  renderBandingDetails(order.banding)}
                              </div>
                            </div>
                          )}
                            ...
                            <div className="space-y-1 text-xs">
                              ...
                              {order.cutting && (
                                <div className="flex justify-between text-blue-600">
                                  ...
                                  <span>
                                    {formatCurrency(order.cutting.total_price)}{" "}
                                    UZS
                                  </span>
                                </div>
                              )}
                              {order.banding && (
                                <div className="flex justify-between text-green-600">
                                  ...
                                  <span>
                                    {formatCurrency(order.banding.total_price)}{" "}
                                    UZS
                                  </span>
                                </div>
                              )}
                              ...
                              {parseFloat(order.discount) > 0 && (
                                <div className="flex justify-between text-green-600">
                                  ...
                                  <span>
                                    -{formatCurrency(order.discount)} UZS
                                  </span>
                                </div>
                              )}
          ...
          {selectedSale && (
            <div className="space-y-3 sm:space-y-4 py-2 sm:py-4">
              <div className="space-y-1 sm:space-y-2">
                ...
                <Input
                  value={selectedSale.receiptNumber}
                  disabled
                  className="text-sm"
                />
              </div>
              ...
              <div className="space-y-1 sm:space-y-2">
                <Label htmlFor="discount" className="text-xs sm:text-sm">
                  {t("discount")}
                </Label>
                <Input
                  id="discount"
              ...
              <div className="space-y-1 sm:space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(value: any) => setEditedPaymentMethod(value)}
                >
              ...
              <div className="border-t pt-3 sm:pt-4">
                ...
                <div className="space-y-2 sm:space-y-3">
                  {selectedSale.items.map((item, index) => (
                    <div
                      key={index}
                      className="flex justify-between items-start p-2 sm:p-3 bg-gray-50 dark:bg-gray-800 rounded-lg"
                    >
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="text-xs text-gray-500 dark:text-gray-400 mt-1 space-y-0.5">
                          ...
                          {item.customWidth && item.customHeight && (
                            <p>
                              {t("dimensions")}: {item.customWidth}x
                              {item.customHeight} mm
                            </p>
                          )}
                          {item.cuttingService && (
                            <p>
                              {t("cuttingService")}:{" "}
                              {formatCurrency(item.cuttingService.total)} UZS
                            </p>
                          )}
                          {item.edgeBandingService && (
                            <p>
                              {t("edgeBandingService")}:{" "}
                              {formatCurrency(item.edgeBandingService.total)}{" "}
                              UZS
                            </p>
                          )}
                        </div>
              ...
              <div className="border-t pt-3 sm:pt-4 space-y-1 sm:space-y-2">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-semibold">
                    {formatCurrency(selectedSale.subtotal)} UZS
                  </span>
                </div>
                {selectedSale.discount > 0 && (
                  <div className="flex justify-between text-sm">
                    ...
                    <span className="font-semibold text-red-600">
                      -{formatCurrency(selectedSale.discount)} UZS
                    </span>
                  </div>
                )}
                <div className="flex justify-between text-base sm:text-lg pt-1">
                  ...
                  <span className="font-bold text-blue-600 dark:text-blue-400">
                    {formatCurrency(selectedSale.total)} UZS
                  </span>
                </div>
              ...
              <div>
                ...
                <div className="space-y-2">
                  {selectedOrder.items.map((item, index) => (
                    <div key={index}>{renderProductDetails(item)}</div>
                  ))}
                </div>
              </div>
                  ...
                  <div>
                    ...
                    <div className="space-y-2">
                      {selectedOrder.cutting &&
                        renderCuttingDetails(selectedOrder.cutting)}
                      {selectedOrder.banding &&
                        renderBandingDetails(selectedOrder.banding)}
                    </div>
                  </div>
                </>
              )}
              ...
              <div className="space-y-1 sm:space-y-2">
                ...
                {selectedOrder.cutting && (
                  <div className="flex justify-between text-sm text-blue-600">
                    ...
                    <span>
                      {formatCurrency(selectedOrder.cutting.total_price)} UZS
                    </span>
                  </div>
                )}
                {selectedOrder.banding && (
                  <div className="flex justify-between text-sm text-green-600">
                    ...
                    <span>
                      {formatCurrency(selectedOrder.banding.total_price)} UZS
                    </span>
                  </div>
                )}
                ...
                {parseFloat(selectedOrder.discount) > 0 && (
                  <div className="flex justify-between text-sm">
                    ...
                    <span className="text-green-600">
                      -{formatCurrency(selectedOrder.discount)} UZS
                    </span>
                  </div>
                )}
                ...
                {selectedOrder.payment_method === "nasiya" && (
                  <>
                    <div className="flex justify-between pt-2 text-sm">
                      ...
                      <span className="text-green-600">
                        {formatCurrency(selectedOrder.covered_amount)} UZS
                      </span>
                    </div>
                    <div className="flex justify-between font-medium text-sm text-yellow-600">
                      ...
                      <span>
                        {formatCurrency(
                          parseFloat(selectedOrder.total_price) -
                            parseFloat(selectedOrder.covered_amount),
                        )}{" "}
                        UZS
                      </span>
                    </div>
            ...
            <div className="space-y-3 sm:space-y-4 py-2 sm:py-4">
              ...
              {selectedCustomer && (
                <div className="rounded-lg bg-gray-50 dark:bg-gray-800 p-2 sm:p-3">
                  ...
                  {selectedCustomer.debt !== undefined &&
                    selectedCustomer.debt > 0 && (
                      <p className="text-xs text-yellow-600 font-medium mt-1">
                        {language === "uz" ? "Qarz" : "Долг"}:{" "}
                        {selectedCustomer.debt.toLocaleString()} UZS
                      </p>
                    )}
                </div>
              )}
              ...
              <div className="space-y-1 sm:space-y-2">
                ...
                <div className="flex gap-2">
                  ...
                  <Select
                    value={discountType}
                    onValueChange={(value: "p" | "c") => setDiscountType(value)}
                  >
                  ...
                  <div className="flex justify-between text-sm text-yellow-600">
                    ...
                    <span>
                      {formatCurrency(
                        Math.max(
                          0,
                          parseFloat(selectedOrder.total_price) - amountPaid,
                        ),
                      )}{" "}
                      UZS
                    </span>
