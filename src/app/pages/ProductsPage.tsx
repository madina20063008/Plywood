...
const mapQualityToApiValue = (
  qualityName: string,
): "standard" | "economic" | "premium" | undefined => {
  const lowerName = qualityName.toLowerCase();
  if (lowerName.includes("premium")) return "premium";
  if (lowerName.includes("economic") || lowerName.includes("эконом"))
    return "economic";
  if (lowerName.includes("standard") || lowerName.includes("стандарт"))
    return "standard";
  return undefined;
};
...
export const ProductsPage: React.FC = () => {
  ...
  const effectiveTotalProducts =
    totalProducts > 0 ? totalProducts : lastTotalProducts.current;
  ...
  useEffect(() => {
    console.log(
      `📦 Page ${currentPage} loaded with ${products.length} products`,
    );
  ...
  const isLoading =
    isFetchingProducts || isFetchingCategories || isFetchingQualities;

  // Calculate displayed range
  const startItem =
    effectiveTotalProducts > 0 ? (currentPage - 1) * limit + 1 : 0;
      ...
      <Card>
        <CardHeader className="p-4 sm:p-6">
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            <div
              className={`${showFilters ? "flex" : "hidden"} sm:flex flex-col sm:flex-row gap-3 sm:gap-2`}
            >
              <Select
              ...
              >
                <SelectTrigger className="w-full sm:w-44 md:w-48">
                  <SelectValue
                    placeholder={language === "uz" ? "Kategoriya" : "Категория"}
                  />
                </SelectTrigger>
                <SelectContent>
              ...
              >
                <SelectTrigger className="w-full sm:w-44 md:w-48">
                  <SelectValue
                    placeholder={language === "uz" ? "Sifat" : "Качество"}
                  />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="all">
            ...
            <>
              ...
              <div className="grid gap-3 sm:gap-4 md:gap-5 lg:gap-6 grid-cols-2 md:grid-cols-3 lg:grid-cols-5">
                {products.map((product) => {
                  const isInCart = displayCart.some(
                    (item: CartItem) => item.product?.id === product.id,
                  );
                  const isAdding = isAddingToCart[product.id];
                  const qualityName = getQualityNameById(
                    product.quality as any,
                  );
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="h-[120px] sm:h-[140px] md:h-[150px] relative aspect-square overflow-hidden bg-gray-100 dark:bg-gray-800">
                        {product.image ? (
                          ...
                          <div
                            ...
                            style={{
                              backgroundColor: product.color || "#e5e7eb",
                            }}
                          />
                        )}
                      </div>
                      ...
                      <CardHeader className="px-3 sm:px-4">
                        <div>
                          <h3
                            className="w-full font-semibold text-sm sm:text-base leading-tight line-clamp-2 min-h-[2.5rem]"
                            title={product.name}
                          >
                            {product.name}
                          </h3>
                      ...
                      <CardFooter className="flex flex-col gap-2 sm:gap-3 pt-0 px-3 sm:px-4 mt-auto">
                        <div className="flex w-full items-center justify-between">
                          <div>
                            <p className="text-md max-md:text-xs font-bold text-blue-600 dark:text-blue-400">
                        ...
                        {isInCart ? (
                          ...
                          <Button
                            ...
                            disabled={
                              (product.stockQuantity || 0) === 0 || isAdding
                            }
                          >
                            {isAdding ? (
